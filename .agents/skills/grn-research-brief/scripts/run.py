...
def main():
    parser = argparse.ArgumentParser(description="GRN Atlas research brief")
    ...
    if args.http:
        data = common.http_post(args.http, "/api/v1/research/brief", payload)
    else:
        sys.path.insert(0, str(common.BACKEND_DIR))
        import main as backend
        req = backend.ResearchBriefRequest(**payload)
        data = common.run_async(backend.research_brief(req))
