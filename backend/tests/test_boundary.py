...
def test_confidence_boundary_reports_supported_and_unsupported_claims(db):
    out = boundary.confidence_boundary(db, ["TF1", "TG1"], intent="experiment")
    lead = out["lead_candidate"]
    assert lead["supported_claims"]
    assert lead["unsupported_claims"]
    assert lead["data_needed"]
