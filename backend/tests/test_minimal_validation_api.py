...
def test_minimal_validation_returns_first_step(client):
    r = client.post("/api/v1/research/minimal-validation", json={"gene_ids": ["TF1", "TG1"], "intent": "experiment"}).json()
    assert r["minimal_first_step"]["experiment"]
    assert r["stop_go_gates"]
