...
def db():
    ...
    conn.executemany(
        ...
        [
            ...
            ("P1", "AN2", "Anthocyanin 2", "petunia", 1, "protein_coding", None, None),
        ],
    )
    conn.executemany(
        ...
        [
            ...
            ("P1", "P1", "activation", 0.6, '["Inferred:Arabidopsis"]', '[]'),
        ],
    )
    ...
    conn.execute("INSERT INTO orthologs VALUES (?,?,?,?,?,?)", ("P1", "TF1", "petunia", "human", "ortholog", 0.8))
    conn.commit()
    yield conn
    conn.close()


def test_validation_plan_has_tracks_and_checklist(db):
    out = validation.build_validation_plan(db, ["TF1", "TG1"], intent="experiment")
    assert out["validation_tracks"]
    assert out["execution_checklist"]
    assert out["decision_gates"]


def test_validation_plan_rnai_leads_with_dsrna(db):
    out = validation.build_validation_plan(db, ["P1"], intent="rnai", species="petunia")
    assert out["validation_tracks"][0]["experiment"] == "dsrna_design"
