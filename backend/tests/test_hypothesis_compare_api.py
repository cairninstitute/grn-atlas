...
def test_hypothesis_compare_returns_winner(client):
    r = client.post("/api/v1/research/hypothesis-compare", json={"gene_ids": ["TF1", "TG1"], "intent": "experiment"}).json()
    assert r["winner"]["gene_id"] == "TF1"
    assert r["pairwise_comparisons"]
