...
def test_study_report_returns_markdown(client):
    r = client.post("/api/v1/research/study-report", json={"gene_ids": ["TF1", "TG1"], "intent": "experiment"}).json()
    assert "packet" in r
    assert "markdown" in r
    assert "## Candidate ranking" in r["markdown"]
