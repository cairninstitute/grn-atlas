...
def test_study_packet_contains_brief_validation_and_citations(db):
    out = packet.build_study_packet(db, ["TF1", "TG1"], intent="experiment")
    assert "brief" in out
    assert "validation_plan" in out
    assert out["citation_bundle"]["sources"]
    assert out["handoff"]["execution_hints"]
