...
def test_evidence_synthesis_returns_candidate_summaries(client):
    r = client.post("/api/v1/research/evidence-synthesis",
                    json={"gene_ids": ["TF1", "TG1"], "intent": "experiment"}).json()
    assert r["lead_candidate"]["gene_id"] == "TF1"
    assert r["candidate_syntheses"]
    assert r["citation_bundle"]["sources"]
