...
def db():
    ...
    conn.executemany(
        ...
        [
            ...
            ("GAP1", "GAP1", "Gap one", "human", 0, "protein_coding", None, None),
        ],
    )
    conn.executemany(
    ...
    conn.execute("INSERT INTO pathway_annotations VALUES (?,?)", ("TF1", "P1"))
    conn.commit()
    yield conn
    conn.close()


def test_evidence_synthesis_returns_pmids_and_citations(db):
    out = synthesis.build_evidence_synthesis(db, ["TF1", "TG1"], intent="experiment")
    assert out["lead_candidate"]["gene_id"] == "TF1"
    assert out["candidate_syntheses"][0]["atlas_pmids"]
    assert out["citation_bundle"]["sources"]


def test_evidence_synthesis_surfaces_caveats_for_sparse_candidates(db):
    out = synthesis.build_evidence_synthesis(db, ["TF1", "GAP1"], intent="experiment", max_candidates=2)
    sparse = next(row for row in out["candidate_syntheses"] if row["gene_id"] == "GAP1")
    assert sparse["contradictory_or_weak_evidence"]
    assert sparse["reporting_caveats"]
