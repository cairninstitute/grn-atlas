...
def test_study_report_contains_markdown_and_packet(db):
    out = reporting.build_study_report(db, ["TF1", "TG1"], intent="experiment")
    assert "packet" in out
    assert "markdown" in out
    assert out["report_sections"]["candidate_table"].startswith("| Gene |")
    assert "# Study report for experiment follow-up" in out["markdown"]
    assert "## Citations" in out["markdown"]
