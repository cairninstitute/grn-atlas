...
def test_study_packet_returns_packet(client):
    r = client.post("/api/v1/research/study-packet", json={"gene_ids": ["TF1", "TG1"], "intent": "experiment"}).json()
    assert "brief" in r
    assert "validation_plan" in r
