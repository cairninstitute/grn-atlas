...
def test_research_brief_has_workflow_and_summary(db):
    out = briefing.build_research_brief(db, ["TF1", "TG1"], intent="experiment")
    assert out["candidate_brief"]
    assert out["workflow_plan"]
    assert out["executive_summary"]


def test_research_brief_rnai_includes_dsrna_step(db):
    out = briefing.build_research_brief(db, ["P1"], intent="rnai", species="petunia")
    assert any(step["action"] == "validate_rnai_design" for step in out["workflow_plan"])
