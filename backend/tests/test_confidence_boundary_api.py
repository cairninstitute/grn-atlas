...
def test_confidence_boundary_returns_boundary_profile(client):
    r = client.post("/api/v1/research/confidence-boundary", json={"gene_ids": ["TF1", "TG1"], "intent": "experiment"}).json()
    assert r["lead_candidate"]["supported_claims"]
    assert r["lead_candidate"]["unsupported_claims"]
